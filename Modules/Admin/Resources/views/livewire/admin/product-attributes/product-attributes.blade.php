...
@if ($showForm == true)
        <livewire:admin.product-attributes.edit />
@elseif ($showDeleted == true)
        <livewire:admin.product-attributes.deleted />
@else
...
?>
          <table class="table table-bordered table-hover table-checkable" id="kt_datatable" style="margin-top: 13px !important">
            <thead>
              <tr>
                <th>#</th>
                <th>{{ trans('ms_lang.size') }}</th>
                <th >{{ trans('ms_lang.color') }}</th>
...
?>
                <tr>
                    <td><?php echo $i; ?></td>
                    <td>{{ Auth::user()->user_lang=='ar' ? @$result->size->name : @$result->size->name_en }}</td>
                    <td>{{ Auth::user()->user_lang=='ar' ? @$result->color->name : @$result->color->name_en }}</td>
                    <td>
