        ...
        <div class="box">
            <div class="box-header">
                <label style="width: 90%!important;text-align: center"> 
                    ...
                    {{$title}}</b>
                    <a  href="{{route('users_details.create',['type'=>$type])}}">
...
@if (count($results))
    ...
    @foreach ($results as $result)
                    ...
                    <tr>
                        ...
                        <td style="padding: 1px;width: 14%">
                            ...
                            <form id="active_ms-form-{{ $result->id }}" action="{{ route('users_details.active_ms', $result->id)}}" method="post" style="display:none">
                                @csrf
                                @method('PUT')
                            </form>
    <?php if($result->is_active ==1){ $styl='success';$act_w='نشـــــط'; }else{$styl='warning'; $act_w='غيرنشط';} ?>
                            ...
                            <!---------- edit Button --->
                            <a href="{{ route('art_min.edit',$result->id) }}" class="btn btn-primary"><i class="glyphicon glyphicon-pencil glyphicon-white"></i></a>
                            <!---------- delete Button --->
