        ...
        <div class="box">
            <div class="box-header">
                <label style="width: 90%!important;text-align: center"> 
                    ...
                    <a  href="{{route('attribute.create',['type'=> $type ])}}">
...
@if ($result->type ==1 && $result->parent_id ==0)
                          <a href="{{route('attribute_values.t',['attribute_id' => $result->id ])}}"><button class="btn btn-info" style="padding: 7px 0;">   قيم {{ $result->name}}</button></a>
@endif
@if(Auth::user()->user_level > 4 || $result->parent_id > 0)
                        <!---------- active & Dis active Button --->
                        <form id="active_ms-form-{{ $result->id }}" action="{{ route('attribute.active_ms', $result->id)}}" method="post" style="display:none">
                            @csrf
                            @method('PUT')
                        </form>
<?php if($result->is_active ==1){ $styl='success';$act_w='نشــــــط'; }else{$styl='warning'; $act_w='غيرنشط';} ?>

                        <a href="{{route('attribute.index')}}" onclick="event.preventDefault();
                        ...
                        <!---------- edit Button --->
                        <a href="{{ route('attribute.edit',$result->id) }}" class="btn btn-primary"><i class="glyphicon glyphicon-pencil glyphicon-white"></i></a>
                        <!---------- delete Button --->
                        <form id="delete-form-{{ $result->id }}" action="{{ route('attribute.destroy',$result->id)}}" method="post" style="display:none">
                            @csrf
                            @method('DELETE')
                        </form>
                        <a href="{{route('attribute.index')}}" onclick="
