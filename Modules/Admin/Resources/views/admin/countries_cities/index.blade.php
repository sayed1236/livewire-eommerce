...
@if ($parent_id > 0)
                    <a  href="{{route('countries_cities.create',['type'=> $type , 'parent_id' => $parent_id ])}}">
...
?>
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                    <th># </th>
                    <th>الاسم / EN Name</th>
                    <th class="{{$dis_det}}">العملة</th>
                    <th class="{{$dis_det}}">مفتاح الدولة</th>
                    <th class="{{$dis_img}}">IMG/الصورة</th>
                    <th class="{{$dis_price}}">سعر التوصيل</th>
                    <th>التاريح</th>
    ...
    @foreach ($results as $result)
                    ...
                    <tr>
                        ...
                        <td class="{{$dis_det}}"><a>{{ $result->currency_code}}</a><br/>{{ $result->currency_code_en}}</td>
                        <td class="{{$dis_det}}" dir="ltr">{{ $result->dail_code }}</td>
                        <td class="{{$dis_img}}"><img src="{{ img_chk_exist($result->img) }}" style="width: 70px; height: 60px" /></td>
                        <td class="{{$dis_price}}">{{ $result->price_move }}</td>
                        <td>{{ $result->created_at }}</td>
                        <td style="padding: 1px;width: 22%">
@if ($result->type ==0 && $result->parent_id ==0)
                          <a href="{{route('countries_cities.t',['type'=> $result->type , 'parent_id' => $result->id ])}}"><button class="btn btn-info" style="padding: 7px 0;">   المحافظات</button></a>
@endif
                        <!---------- active & Dis active Button --->
                        <form id="active_ms-form-{{ $result->id }}" action="{{ route('countries_cities.active_ms', $result->id)}}" method="post" style="display:none">
                            @csrf
                            @method('PUT')
                        </form>
<?php if($result->is_active ==1){ $styl='success';$act_w='نشــــــط'; }else{$styl='warning'; $act_w='غيرنشط';} ?>

                        <a href="{{route('countries_cities.index')}}" onclick="event.preventDefault();
                        ...
                        <!---------- edit Button --->
                        <a href="{{ route('countries_cities.edit',$result->id) }}" class="btn btn-primary"><i class="glyphicon glyphicon-pencil glyphicon-white"></i></a>
                        <!---------- delete Button --->
                        <form id="delete-form-{{ $result->id }}" action="{{ route('countries_cities.destroy',$result->id)}}" method="post" style="display:none">
                            @csrf
                            @method('DELETE')
                        </form>
                        <a href="{{route('countries_cities.index')}}" onclick="
