        ...
        <div class="box">
            <div class="box-header">
                <p style="width: 90%!important;text-align: center"> 
                    ...
                    <a  href="{{route('portfolios.create',['type'=>$type,'cat_id'=>$cat_id])}}">
...
@if (count($results))
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                    ...
                    <th>Project Name </th>
                    <th>Client Name</th>
                    <th class="{{$dis_img}}">Main Image</th>
                    <th class="{{$dis_det}}">Project Description</th>
                    <th>date added</th>
                    <th>Action</th>
                </tr>
                </thead>
    ...
    @foreach ($results as $result)
                    ...
                    <tr>
                        ...
                        <td style="padding: 1px;width: 17%">
                            ...
                            <form id="active_ms-form-{{ $result->id }}" action="{{ route('portfolios.active_ms', $result->id)}}" method="post" style="display:none">
                                @csrf
                                @method('PUT')
                            </form>
                            <?php if($result->is_active ==1){ $styl='success';$act_w=__('ms_lang.active'); }else{$styl='warning'; $act_w=__('ms_lang.disactive');} ?>

                            <a href="{{route('portfolios.index')}}" onclick="event.preventDefault();
                            ...
                            <!---------- edit Button --->
                            <a href="{{ route('portfolios.edit',$result->id) }}" class="btn btn-primary"><i class="glyphicon glyphicon-pencil glyphicon-white"></i>{{$txt_det}}</a>
                            <!---------- delete Button --->
                            <form id="delete-form-{{ $result->id }}" action="{{ route('portfolios.destroy',$result->id)}}" method="post" style="display:none">
                                @csrf
                                @method('DELETE')
                            </form>
                            <a href="{{route('portfolios.index')}}" onclick="if(confirm('هل انت متأكد من الحذف؟')){
