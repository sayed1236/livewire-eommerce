...
class Search extends Component
{
    ...
    public $subcategories,$mainCategoryVisible=true,$min_price,$max_price,$category_id,$sortprice,$attribute_id,$show=true,$att_id=[] ;
   
    public $searchvalue='',$category;
    ...
    public function render()
    {
        ...
        // $vendors = [];
        if ($this->category == 'products' && strlen($this->searchvalue)>=1 ) {
            // $products = Product::with(['latestProductStock','discounts_product'])->where('name', 'like', '%' . $this->searchvalue . '%')->limit(6)->get();
 
            $products = Product::join('products_stocks', 'products_stocks.product_id', '=', 'products.id')
            ->when($this->sortprice !== null, function ($qw) {
                if ($this->sortprice == 'date') {
                    $qw->orderBy('products_stocks.created_at', 'desc');
                } else {
                    $qw->orderBy('products_stocks.selling_price', $this->sortprice);
                }
            })
            ...
            ->with(['discounts_product', 'latestProductStock' => function ($q) {
            ...
            ->select([
                ...
                'products.created_at',
                DB::raw('MAX(products_stocks.selling_price) as max_selling_price') // Using MAX() for non-grouped column
            ])
            ->groupBy('products.id', 'products.name', 'products.img', 'products.category_id', 'products.created_at')
            ->where('name', 'like', '%' . $this->searchvalue . '%')->limit(6)->get();
    }
    ...
    ->get();    
        ...
        ->get();   
        $attributes = Category::with([ 'attributes' => function ($q) {
         $q->with('attribute_values')->where('parent_id', 0);
     }])
        ->when($descendantCategoryIds, function ($query) use ($descendantCategoryIds) {
         $query->whereIn('id',$descendantCategoryIds)->get();
        })->get();  
        
        return view('trader::livewire.search.search',compact('products','categories','attributes') )->extends('trader::components.layouts.app');
    }
